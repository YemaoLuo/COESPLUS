...
<script lang="ts" setup>
  ...
  import { getTableList, courseAction } from '@/api/table/enrollinglist';
  //
  ...
  async function addCourse(record: Recordable) {
    const response = await courseAction({ id: record.id, state: 0 });
    console.log(response);
    if (response.code == 200) message.success('加选成功');
    else if (response.code == 400) message.error(response.message.toString());
  }
  ...
  function cancelCourse(record: Recordable) {
    ...
    dialog.info({
      title: '警告',
      content: `您真的要退选课程“${record.name}”吗`,
      positiveText: '退选',
      negativeText: '取消',
      onPositiveClick: async () => {
        const response = await courseAction({ id: record.id, state: 1 });
        console.log(response);
        if (response.code == 200) message.success('退选成功');
        else if (response.code == 400) message.success(response.message.toString());
      },
      onNegativeClick: () => {},
    });
  }
