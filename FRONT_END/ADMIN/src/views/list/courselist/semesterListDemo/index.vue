<template>
  <n-card :bordered="false" class="proCard">
    ...
    <!-------------------------------------------课程设置----------------------------------------------------->
    <n-space>
      <n-modal v-model:show="showTableModal" preset="card" transform-origin="center" title="学期课程">
        <BasicTable
...
<script lang="ts" setup>
  ...
  function handleDelete(record) {
    ...
    dialog.info({
      ...
      onPositiveClick: async () => {
        const response = await DeleteSemester(record.id);
        if (response.code == 200) {
  ...
  function handleCourseDelete(record) {
    ...
    dialog.info({
      ...
      negativeText: '取消',
      onPositiveClick: async () => {
        const response = await DeleteSemesterCourse(record.id);
        if (response.code == 200) {
          message.success('删除成功');
        }
        if (response.code == 400) {
  ...
  function handleCreditDelete(record) {
    ...
    dialog.info({
      ...
      negativeText: '取消',
      onPositiveClick: async () => {
        const response = await DeleteSemesterCredit(record.id);
        if (response.code == 200) {
          message.success('删除成功');
        }
        if (response.code == 400) {
  ...
  const schemas: FormSchema[] = [
    {
      ...
      label: '学期',
      componentProps: {
        placeholder: '请输入该学期编号',
        onInput: (e: any) => {
          formParams.semester = e;
        },
      },
    },
    {
